...
public class SymbolTable extends GJDepthFirst<ReturnValue, FunctionInfo> {
  ...
  public SymbolTable(){
   ...
   public ReturnValue visit(MainClass n, FunctionInfo argu) throws Exception {
      ...
      if(n.f14.present()){
         for(int i = 0 ; i < n.f14.nodes.size(); i++ ) {
          ...
          if(MainTable.identifiers.containsKey(field.name) || MainTable.arguments.containsKey(field.name)){
            throw new Exception("error: variable " + field.name + " is already defined in method main(String[])");
          }
   ...
   public ReturnValue visit(ClassDeclaration n, FunctionInfo argu) throws Exception {
      ...
      if(n.f3.present()){
        for(int i = 0 ; i < n.f3.nodes.size(); i++ ) {
            ...
            if(info.members.containsKey(field.name)){
               throw new Exception("error: " + "variable " + field.name + " is already defined in class " + info.name);
            }
      ...
      if(n.f4.present()){
        for(int i = 0 ; i < n.f4.nodes.size(); i++ ) {
            ...
            if(info.functions.containsKey(funct.name)){
                throw new Exception("error: method " + funct.name + " is already defined in class " + info.name);
            }
            else{
      ...
      if(ClassTable.containsKey(info.name)){
        throw new Exception("error: duplicate class: " + info.name);
      }
   ...
   public ReturnValue visit(ClassExtendsDeclaration n, FunctionInfo argu) throws Exception {
      ...
      if(!ClassTable.containsKey(info.parent)){
        throw new Exception("error: parent class " + info.parent + " must be defined before " + info.name);
      }
      if(n.f5.present()){
   ...
   public ReturnValue visit(MethodDeclaration n, FunctionInfo argu) throws Exception {
      ...
      if(n.f7.present()){
        for(int i = 0 ; i < n.f7.nodes.size(); i++ ) {
            ...
            FieldInfo field = (FieldInfo) val.typeValue;
            if(funct.identifiers.containsKey(field.name) || funct.arguments.containsKey(field.name)){
              throw new Exception("error: variable " + field.name + " is already defined in method " + funct.name + "(" + argStr + ")");
            }
   ...
   public ReturnValue visit(FormalParameterList n, FunctionInfo argu) throws Exception {
      ...
      if(argu.arguments.containsKey(field.name)){
          throw new Exception("error: variable " + field.name + " is already defined in method " + argu.name);
      }
